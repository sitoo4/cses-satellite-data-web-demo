...
function uploadPayload(fileCount: number) {
  return {
    ...
    per_file_records: Array.from({ length: fileCount }, (_, index) => ({
      ...
      filename: `CSES_01_HPM_5_L02_A2_${index + 1}.h5`,
      size_bytes: 2048,
      duplicate_of: index === 1 && fileCount === 2 ? "CSES_01_HPM_5_L02_A2_1.h5" : null,
      status: "ok",
      hpm_product: "HPM_5",
      sample_count: 4,
      time_parseable: true,
    ...
    sorted_files: [
      {
        upload_id: "file_1",
        filename: "CSES_01_HPM_5_L02_A2_1.h5",
        hpm_product: "HPM_5",
...
describe("App upload workflow", () => {
  ...
  it("uploads one file and switches to SINGLE FILE mode", async () => {
    ...
    expect(screen.getAllByText("CSES_01_HPM_5_L02_A2_1.h5").length).toBeGreaterThan(0);
    ...
    expect(document.querySelector(".crop-label-cell > small")).toHaveTextContent("裁剪范围来自后端可用时间段。按北京时间选择。");
  ...
  it("shows feature statistics and downloads exported data/current plot", async () => {
    ...
    expect(screen.getAllByText("/FLAG_MT: 正常=2, 磁力矩器干扰=2").length).toBeGreaterThan(0);
