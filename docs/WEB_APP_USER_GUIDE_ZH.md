...
本文档说明 `/Volumes/Elements/satellite_data_web` 当前网页的实际使用方式和功能边界。它描述的是当前前端页面，不是正式科学产品说明，也不是 CSES 数据处理上线文档。
...
- `Cluster 频谱保留`：读取 `/Volumes/Elements/data/idlpython_v2` 中已经存在的 Cluster C1 处理产品，并在 Web 项目自己的 `outputs/generated_plots/cluster/` 下再生成图件。
- `CSES 磁场实验`：读取本机 CSES-01 HPM H5 文件索引和检查结果，只做 H5 字段读取、预览、统计和导出检查。当前不从前端生成 CSES 图件。

重要边界：

- 不修改 `/Volumes/Elements/data/idlpython_v2` 中的 Cluster 原始处理程序和已有数据结构。
...
`磁场总览` 当前按 `/Volumes/Elements/data/idlpython_v2/plot_daily_quicklook.py` 的 B quicklook 面板配方生成。
...
```text
/Volumes/Elements/satellite_data_web/outputs/
```
