...
function segmentShadowColor(color) {{ const match = /^#?([a-f\\d]{{2}})([a-f\\d]{{2}})([a-f\\d]{{2}})$/i.exec(color || ""); if(!match) return "rgba(128,35,32,.78)"; const r=Math.max(86, parseInt(match[1],16)-62), g=Math.max(24, parseInt(match[2],16)-56), b=Math.max(28, parseInt(match[3],16)-50); return `rgba(${{r}},${{g}},${{b}},.78)`; }}
function drawOrbitStroke(points, color) {{ if(points.length<2)return; line(points, segmentShadowColor(color), 8.2); line(points, "rgba(255,226,205,.82)", 5.8); line(points, color, 4.2); ctx.save(); ctx.setLineDash([2, 8]); ctx.lineCap="round"; ctx.beginPath(); ctx.moveTo(points[0].x,points[0].y); for(let i=1;i<points.length;i++)ctx.lineTo(points[i].x,points[i].y); ctx.strokeStyle=segmentHighlightColor(color); ctx.lineWidth=2.2; ctx.stroke(); ctx.restore(); }}
